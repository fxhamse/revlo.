...
export async function GET(request: Request) {
  try {
    ...
    expenses.forEach(exp => {
      ...
      // Category breakdown
      expenseCategoryBreakdown[exp.category] = (expenseCategoryBreakdown[exp.category] || 0) + amount;

      // Monthly trend
      const monthYear = exp.expenseDate.toLocaleString('en-US', { month: 'short', year: 'numeric' });
    ...
    const categoryBreakdownData = Object.keys(expenseCategoryBreakdown).map(key => ({
        name: key,
    ...
    return NextResponse.json(
      {
        ...
        expenses: expenses.map(exp => ({ // Return original expenses with converted amount
            ...exp,
