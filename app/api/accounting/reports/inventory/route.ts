...
export async function GET(request: Request) {
  try {
    ...
    const totalValueAtCost = filteredItems.reduce((sum, item) => sum + (item.inStock * item.purchasePrice.toNumber()), 0);
    const totalPotentialSellingValue = filteredItems.reduce((sum, item) => sum + (item.inStock * item.sellingPrice.toNumber()), 0);
    ...
    const categoryDistributionData = Object.keys(categoryDistributionMap).map(key => ({
      name: key,
    ...
    return NextResponse.json(
      {
        ...
        items: filteredItems.map(item => ({ // Return original items with converted Decimal to Number
            ...item,
