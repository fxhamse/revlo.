...
import type { TransactionType } from '@prisma/client'; // Import TransactionType
...
export async function GET(request: Request) {
  try {
    ...
    const accounts = await prisma.account.findMany({
      // where: { companyId: companyId }, // Mustaqbalka, ku dar filter-kan
      orderBy: { name: 'asc' },
    });
    ...
    const transactions = await prisma.transaction.findMany({
      where: {
        ...
        type: typeFilter && ['INCOME', 'EXPENSE', 'TRANSFER_IN', 'TRANSFER_OUT', 'DEBT_TAKEN', 'DEBT_REPAID'].includes(typeFilter) ? typeFilter as TransactionType : undefined,
    ...
    const totalBankBalance = accounts.filter(acc => acc.type === 'BANK' || acc.type === 'MOBILE_MONEY').reduce((sum, acc) => sum + acc.balance.toNumber(), 0);
    const totalCashBalance = accounts.filter(acc => acc.type === 'CASH').reduce((sum, acc) => sum + acc.balance.toNumber(), 0);
    ...
    const accountDistributionData = accounts.map(acc => ({ name: acc.name, value: acc.balance.toNumber() })).filter(item => item.value > 0);
    ...
    return NextResponse.json(
      {
        ...
        transactions: transactions.map(trx => ({ // Return original transactions with converted Decimal to Number
            ...trx,
            amount: trx.amount.toNumber(),
        })),
        accounts: accounts.map(acc => ({ // Return original accounts with converted Decimal to Number
            ...acc,
