...
export async function GET(request: Request) {
  try {
    ...
    // Debts
    const outstandingDebts = await prisma.expense.aggregate({
      _sum: { amount: true },
      where: { companyId, category: 'Debt' },
    });
    // Receivable debts: sum of payments with status 'Pending' for projects of this company
    const receivableDebts = await prisma.payment.aggregate({
      _sum: { amount: true },
      where: {
        project: { companyId },
      },
    ...
    return NextResponse.json({
      stats: {
        ...
        projectExpenses: projectExpenses._sum.amount || 0,
        outstandingDebts: outstandingDebts._sum.amount || 0,
        receivableDebts: receivableDebts?._sum?.amount || 0,
        fixedAssetsValue: fixedAssets._sum.value || 0,
        totalBankBalance: totalBankBalance._sum.balance || 0,
        totalCashBalance: totalCashBalance._sum.balance || 0,
        shareholdersEquity: 0, // Not available in schema, set to 0 or calculate if needed
      },
    }, { status: 200 });
  } catch (error) {
