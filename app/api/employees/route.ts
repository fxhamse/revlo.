...
export async function POST(request: Request) {
  try {
    ...
    if (category === 'PROJECT') {
      // Project employee: create ProjectLabor record
      const { projectId, employeeId, workDescription, agreedWage, paidAmount, dateWorked } = body;
      if (!projectId || !employeeId || !workDescription || typeof agreedWage !== 'number' || !dateWorked) {
        return NextResponse.json(
          { message: 'Fadlan buuxi dhammaan beeraha waajibka ah ee shaqaalaha mashruuca.' },
          { status: 400 }
        );
      }
      // Create ProjectLabor record
      // Get employee name for record
      const employee = await prisma.employee.findUnique({ where: { id: employeeId } });
      const labor = await prisma.projectLabor.create({
        data: {
          employeeName: employee ? employee.fullName : '',
          workDescription,
          agreedWage: new Decimal(agreedWage),
          paidAmount: new Decimal(paidAmount || 0),
          remainingWage: new Decimal(agreedWage - (paidAmount || 0)),
      ...
      return NextResponse.json(
        { message: 'Shaqaalaha mashruuca si guul leh ayaa loo daray!', labor },
        { status: 201 }
      );
    } else {
