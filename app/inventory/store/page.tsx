// app/inventory/store/page.tsx - Inventory Store Page (10000% Design - API Integration & Enhanced)
'use client';
...
import { 
  ...
  Info as InfoIcon, AlertTriangle, ClipboardList, RefreshCw, Loader2
} from 'lucide-react';
...
export default function InventoryStorePage() {
  ...
  const [viewMode, setViewMode] = useState<'list' | 'charts' | 'cards'>('list'); // Added 'cards' view mode
  ...
  // Filter options
  const categories = ['All', ...Array.from(new Set(safeInventoryItems.map(item => item.category)))];
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex space-x-3">
          <Link href="/inventory/store/add" className="bg-primary text-white py-2.5 px-6 rounded-lg font-bold text-lg hover:bg-blue-700 transition duration-200 shadow-md flex items-center">
            <Plus size={20} className="mr-2" /> Ku Dar Alaab
          </Link>
          <button onClick={fetchInventoryItems} className="bg-secondary text-white py-2.5 px-6 rounded-lg font-bold text-lg hover:bg-green-600 transition duration-200 shadow-md flex items-center">
