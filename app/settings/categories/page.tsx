...
const dummyExpenseCategories = [
  { id: 'cat001', name: 'Material', type: 'Project', description: 'Kharashyada alaabta ceeriin ah ee mashaariicda.', lastUpdated: '2025-07-20' },
  { id: 'cat002', name: 'Labor', type: 'Project', description: 'Kharashyada shaqaalaha iyo mushaharkooda mashruuca.', lastUpdated: '2025-07-19' },
  { id: 'cat003', name: 'Transport', type: 'Project', description: 'Kharashyada gaadiidka alaabta iyo shaqaalaha.', lastUpdated: '2025-07-18' },
  { id: 'cat004', name: 'Salary', type: 'Company', description: 'Mushaharka shaqaalaha guud ee shirkadda.', lastUpdated: '2025-07-23' },
  { id: 'cat005', name: 'Office Rent', type: 'Company', description: 'Kirada xafiiska iyo meelaha shaqada.', lastUpdated: '2025-07-21' },
  { id: 'cat006', name: 'Debt', type: 'Company', description: 'Deynaha la qaatay.', lastUpdated: '2025-07-16' },
  { id: 'cat007', name: 'Debt Repayment', type: 'Company', description: 'Dib u bixinta deynaha.', lastUpdated: '2025-07-15' },
  { id: 'cat008', name: 'Other', type: 'General', description: 'Kharashyada kale ee aan ku jirin noocyada kale.', lastUpdated: '2025-07-24' },
];

// --- Expense Category Table Row Component ---
const ExpenseCategoryRow: React.FC<{ category: typeof dummyExpenseCategories[0]; onEdit: (id: string) => void; onDelete: (id: string) => void }> = ({ category, onEdit, onDelete }) => (
  <tr className="hover:bg-lightGray dark:hover:bg-gray-700 transition-colors duration-150 border-b border-lightGray dark:border-gray-700 last:border-b-0">
    ...
    <td className="p-4 whitespace-nowrap text-mediumGray dark:text-gray-300">{new Date(category.lastUpdated).toLocaleDateString()}</td>
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 animate-fade-in">
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-lg animate-fade-in-up">
      <div className="flex justify-between items-center mb-4 border-b pb-3 border-lightGray dark:border-gray-700">
        ...
        <button onClick={onClose} className="text-mediumGray dark:text-gray-400 hover:text-redError transition-colors">
          <X size={24} />
        </button>
      </div>
      {children}
...
interface CategoryFormProps {
  ...
  onCancel: () => void;
  editingCategory?: typeof dummyExpenseCategories[0] | null; // Optional for editing
}
const CategoryForm: React.FC<CategoryFormProps> = ({ onSubmit, onCancel, editingCategory }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => { // Simulate API call
      onSubmit({ id: editingCategory?.id, name, type, description });
      setLoading(false);
    }, 1000);
  };
...
export default function ExpenseCategoriesSettingsPage() {
  const [categories, setCategories] = useState(dummyExpenseCategories);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterType, setFilterType] = useState('All');
  const [showAddEditModal, setShowAddEditModal] = useState(false);
  const [editingCategory, setEditingCategory] = useState<typeof dummyExpenseCategories[0] | null>(null);
  ...
  const handleAddCategory = (newCategoryData: any) => {
    console.log('Adding category:', newCategoryData);
    setCategories(prev => [...prev, { ...newCategoryData, id: `cat${prev.length + 1}`, lastUpdated: new Date().toISOString().split('T')[0] }]);
    setToastMessage({ message: 'Nooca si guul leh ayaa loo daray!', type: 'success' });
    setShowAddEditModal(false);
  };

  const handleEditCategory = (updatedCategoryData: any) => {
    console.log('Updating category:', updatedCategoryData);
    setCategories(prev => prev.map(cat => cat.id === updatedCategoryData.id ? { ...updatedCategoryData, lastUpdated: new Date().toISOString().split('T')[0] } : cat));
  ...
  const handleDeleteCategory = (id: string) => {
    if (window.confirm('Ma hubtaa inaad doonayso inaad tirtirto noocan kharashka?')) { // Use custom modal later
