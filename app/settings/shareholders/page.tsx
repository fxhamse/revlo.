...
const dummyShareholders = [
  { id: 'sh001', name: 'Axmed Cali', email: 'axmed.ali@example.com', sharePercentage: 50, profitSplit: 25000, joinedDate: '2020-01-01' },
  { id: 'sh002', name: 'Faadumo Maxamed', email: 'faadumo.m@example.com', sharePercentage: 30, profitSplit: 15000, joinedDate: '2021-03-10' },
  { id: 'sh003', name: 'Cali Xasan', email: 'cali.h@example.com', sharePercentage: 20, profitSplit: 10000, joinedDate: '2022-05-15' },
];

// --- Shareholder Table Row Component ---
const ShareholderRow: React.FC<{ shareholder: typeof dummyShareholders[0]; onEdit: (id: string) => void; onDelete: (id: string) => void }> = ({ shareholder, onEdit, onDelete }) => (
...
const ShareholderCard: React.FC<{ shareholder: typeof dummyShareholders[0]; onEdit: (id: string) => void; onDelete: (id: string) => void }> = ({ shareholder, onEdit, onDelete }) => (
...
interface ShareholderFormProps {
  ...
  onCancel: () => void;
  editingShareholder?: typeof dummyShareholders[0] | null; // Optional for editing
}
const ShareholderForm: React.FC<ShareholderFormProps> = ({ onSubmit, onCancel, editingShareholder }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => { // Simulate API call
      onSubmit({ id: editingShareholder?.id, name, email, sharePercentage, joinedDate });
      setLoading(false);
    }, 1000);
  };
...
export default function ShareholdersSettingsPage() {
  const [shareholders, setShareholders] = useState(dummyShareholders);
  const [searchTerm, setSearchTerm] = useState('');
  const [showAddEditModal, setShowAddEditModal] = useState(false);
  const [editingShareholder, setEditingShareholder] = useState<typeof dummyShareholders[0] | null>(null);
  ...
  const handleAddShareholder = (newShareholderData: any) => {
    console.log('Adding shareholder:', newShareholderData);
    setShareholders(prev => [...prev, { ...newShareholderData, id: `sh${prev.length + 1}`, profitSplit: 0 }]); // Profit split is 0 initially
    setToastMessage({ message: 'Saamileyda si guul leh ayaa loo daray!', type: 'success' });
    setShowAddEditModal(false);
  };

  const handleEditShareholder = (updatedShareholderData: any) => {
    console.log('Updating shareholder:', updatedShareholderData);
    setShareholders(prev => prev.map(sh => sh.id === updatedShareholderData.id ? { ...updatedShareholderData, profitSplit: sh.profitSplit } : sh)); // Keep old profit split
  ...
  const handleDeleteShareholder = (id: string) => {
    if (window.confirm('Ma hubtaa inaad doonayso inaad tirtirto saamileyda?')) { // Use custom modal later
      console.log('Deleting shareholder:', id);
