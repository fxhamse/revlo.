...
const dummyFixedAssets = [
  { id: 'asset001', name: 'CNC Machine', type: 'Machinery', value: 25000.00, purchaseDate: '2023-01-15', assignedTo: 'Factory', status: 'Active', lastUpdated: '2025-07-20' },
  { id: 'asset002', name: 'Office Desk Set', type: 'Furniture', value: 1200.00, purchaseDate: '2024-03-01', assignedTo: 'Office', status: 'Active', lastUpdated: '2025-07-21' },
  { id: 'asset003', name: 'Delivery Van', type: 'Vehicle', value: 18000.00, purchaseDate: '2023-06-10', assignedTo: 'Transport', status: 'Active', lastUpdated: '2025-07-22' },
  { id: 'asset004', name: 'Drill Press', type: 'Tool', value: 500.00, purchaseDate: '2024-09-20', assignedTo: 'Furniture Project A', status: 'Assigned', lastUpdated: '2025-07-23' },
  { id: 'asset005', name: 'Laptop (Admin)', type: 'Electronics', value: 1500.00, purchaseDate: '2025-01-05', assignedTo: 'Office', status: 'Active', lastUpdated: '2025-07-24' },
];
...
const FixedAssetRow: React.FC<{ asset: typeof dummyFixedAssets[0]; onEdit: (id: string) => void; onDelete: (id: string) => void }> = ({ asset, onEdit, onDelete }) => (
  <tr className="hover:bg-lightGray dark:hover:bg-gray-700 transition-colors duration-150 border-b border-lightGray dark:border-gray-700 last:border-b-0">
    ...
    <td className="p-4 whitespace-nowrap text-mediumGray dark:text-gray-300">{new Date(asset.lastUpdated).toLocaleDateString()}</td>
...
interface AssetFormProps {
  ...
  onCancel: () => void;
  editingAsset?: typeof dummyFixedAssets[0] | null; // Optional for editing
}
const AssetForm: React.FC<AssetFormProps> = ({ onSubmit, onCancel, editingAsset }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => { // Simulate API call
      onSubmit({ id: editingAsset?.id, name, type, value, purchaseDate, assignedTo });
      setLoading(false);
    }, 1000);
  };
...
export default function FixedAssetsSettingsPage() {
  ...
  const handleAddAsset = (newAssetData: any) => {
    ...
    setAssets(prev => [...prev, { ...newAssetData, id: `asset${prev.length + 1}`, status: 'Active', lastUpdated: new Date().toISOString().split('T')[0] }]);
  ...
  const handleEditAsset = (updatedAssetData: any) => {
    ...
    setAssets(prev => prev.map(asset => asset.id === updatedAssetData.id ? { ...updatedAssetData, lastUpdated: new Date().toISOString().split('T')[0] } : asset));
  ...
  const handleDeleteAsset = (id: string) => {
    if (window.confirm('Ma hubtaa inaad doonayso inaad tirtirto hantidan?')) { // Use custom modal later
