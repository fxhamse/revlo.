// app/vendors/add/page.tsx - Add New Vendor Page (10000% Design - API Integration)
'use client';
...
import { 
  ...
  ArrowLeft, Tag, Briefcase as BriefcaseIcon, ChevronRight // Added ChevronRight for dropdown
} from 'lucide-react';
...
export default function AddVendorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setToastMessage({ message: data.message || 'Iibiyaha si guul leh ayaa loo daray!', type: 'success' });
        router.push('/vendors'); // Redirect to vendors list on success
      } else {
        setToastMessage({ message: data.message || 'Cilad ayaa dhacday marka iibiyaha la darayay.', type: 'error' });
      }
    } catch (error: any) {
      console.error('Vendor Add API error:', error);
      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md animate-fade-in-up">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            ...
            {loading ? 'Diiwaan Gelinaya Iibiye...' : 'Diiwaan Geli Iibiye'}
          </button>
        </form>
      </div>
      {toastMessage && (
