  ...
  <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-md animate-fade-in-up border-l-4 border-primary relative">
    <div className="flex justify-between items-start mb-3">
      <h4 className="font-semibold text-darkGray dark:text-gray-100 text-lg flex items-center space-x-2">
        <User size={20} className="text-primary"/> <span>{customer.name}</span>
      </h4>
    ...
    <p className="text-sm text-mediumGray dark:text-gray-400 mb-1 flex items-center space-x-2">
        <Tag size={14}/> <span>Nooca: {customer.type}</span>
    </p>
    {customer.companyName && (
        <p className="text-sm text-mediumGray dark:text-gray-400 mb-1 flex items-center space-x-2">
            <Building size={14}/> <span>Shirkad: {customer.companyName}</span>
        </p>
    )}
    <p className="text-sm text-mediumGray dark:text-gray-400 mb-1 flex items-center space-x-2">
        {customer.email ? <Mail size={14}/> : <XCircle size={14} className="text-redError"/>} <span>Email: {customer.email || 'N/A'}</span>
    </p>
    <p className="text-sm text-mediumGray dark:text-gray-400 mb-1 flex items-center space-x-2">
        {customer.phone ? <Phone size={14}/> : <XCircle size={14} className="text-redError"/>} <span>Taleefan: {customer.phone || 'N/A'}</span>
    </p>
    <p className="text-sm text-mediumGray dark:text-gray-400 mb-1 flex items-center space-x-2">
        {customer.address ? <MapPin size={14}/> : <XCircle size={14} className="text-redError"/>} <span>Cinwaan: {customer.address || 'N/A'}</span>
    </p>
    <p className="text-sm text-mediumGray dark:text-gray-400 flex items-center space-x-2">
        <Calendar size={14}/> <span>Diiwaan Gashan: {new Date(customer.createdAt).toLocaleDateString()}</span>
    </p>
    <Link href={`/customers/${customer.id}`} className="mt-3 inline-block text-primary hover:underline text-sm font-medium">
    ...
    <Layout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8 animate-fade-in-up">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md text-center">
          <h4 className="text-lg font-semibold text-mediumGray dark:text-gray-400">Wadarta Macaamiisha</h4>
          <p className="text-3xl font-extrabold text-primary">{totalCustomersCount}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md text-center">
          <h4 className="text-lg font-semibold text-mediumGray dark:text-gray-400">Macaamiisha Shakhsiga ah</h4>
          <p className="text-3xl font-extrabold text-secondary">{individualCustomersCount}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md text-center">
          <h4 className="text-lg font-semibold text-mediumGray dark:text-gray-400">Macaamiisha Shirkadaha</h4>
          <p className="text-3xl font-extrabold text-accent">{companyCustomersCount}</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md mb-8 flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 animate-fade-in-up">
        ...
        <div className="flex space-x-2 w-full md:w-auto justify-center">
            <button title="List view" onClick={() => setViewMode('list')} className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-primary text-white' : 'bg-lightGray dark:bg-gray-700 text-mediumGray dark:text-gray-400'} hover:bg-primary/80 dark:hover:bg-gray-600 transition-colors duration-200`}>
                <List size={20} />
            </button>
            <button title="Cards view" onClick={() => setViewMode('cards')} className={`p-2 rounded-lg ${viewMode === 'cards' ? 'bg-primary text-white' : 'bg-lightGray dark:bg-gray-700 text-mediumGray dark:text-gray-400'} hover:bg-primary/80 dark:hover:bg-gray-600 transition-colors duration-200`}>
                <LayoutGrid size={20} />
            </button>
        </div>
      </div>
