...
interface Project {
    ...
    expenses: { id: string; description: string; amount: number; category: string; expenseDate: string; }[];
...
const ProjectDetailsPage: React.FC = () => {
    ...
    const remainingAmount = totalValue - totalAdvance;
        ...
        <Layout>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md">
                  ...
                  <p className="text-2xl font-bold text-darkGray dark:text-gray-100">${totalValue.toLocaleString()}</p>
                </div>
                <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md">
                  <p className="text-sm text-secondary">Total Advance</p>
                  <p className="text-2xl font-bold text-secondary">${totalAdvance.toLocaleString()}</p>
                </div>
                <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md">
                  <p className="text-sm text-secondary">Total Expenses</p>
                  <p className="text-2xl font-bold text-secondary">${totalExpenses.toLocaleString()}</p>
                </div>
                <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md">
                  <p className="text-sm text-redError">Total Remaining</p>
                  <p className="text-2xl font-bold text-redError">${remainingAmount.toLocaleString()}</p>
                </div>
                        ...
                        {Object.entries(expensesByCategory).map(([cat, exps]) => (
                            <div key={cat} className="bg-lightGray/30 rounded-lg p-2">
                                ...
                                <div className="flex justify-between items-center mb-1">
                                    <span className="font-semibold text-darkGray text-sm">Total: <span className="text-redError">-${exps.reduce((s, e) => s + (typeof e.amount === 'number' ? e.amount : parseFloat(e.amount as any) || 0), 0).toLocaleString()}</span></span>
                                </div>
                                <div className="space-y-2">
                                    {exps.map(exp => (
                                        <div key={exp.id} className='bg-white p-3 rounded-lg flex justify-between items-center shadow-sm'>
                                            ...
                                            <p className='font-bold text-lg text-redError'>-${(typeof exp.amount === 'number' ? exp.amount : parseFloat(exp.amount as any) || 0).toLocaleString()}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="hidden md:block">
                        {Object.entries(expensesByCategory).map(([cat, exps]) => (
                            <div key={cat} className="mb-6">
                                ...
                                <div className="flex justify-between items-center mb-1">
                                    <span className="font-semibold text-darkGray text-base">Total: <span className="text-redError">-${exps.reduce((s, e) => s + (typeof e.amount === 'number' ? e.amount : parseFloat(e.amount as any) || 0), 0).toLocaleString()}</span></span>
                                </div>
                                    ...
                                    <tbody>
                                        {exps.map(exp => (
                                            <tr key={exp.id}>
                                                ...
                                                <td className='text-right font-semibold text-redError'>-${(typeof exp.amount === 'number' ? exp.amount : parseFloat(exp.amount as any) || 0).toLocaleString()}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <div className="space-y-2">
                                {exps.map(exp => (
                                    <div key={exp.id} className="bg-white p-3 rounded-lg flex flex-col gap-1 shadow-sm">
                                        ...
                                        <span className="text-right font-semibold text-redError">-${(typeof exp.amount === 'number' ? exp.amount : parseFloat(exp.amount as any) || 0).toLocaleString()}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                                        ...
                                        <tbody className="divide-y divide-lightGray">
                                            {project.materialsUsed.map((mat) => (
                                                <tr key={mat.id}>
                                                    ...
                                                    <td className="px-3 py-2 text-right text-mediumGray">${Number(mat.costPerUnit).toLocaleString()}</td>
                                                    <td className="px-3 py-2 text-right text-mediumGray">{mat.unit}</td>
                                                    <td className="px-3 py-2 text-right font-semibold text-darkGray">${(mat.quantityUsed * Number(mat.costPerUnit)).toLocaleString()}</td>
                    ...
                    {['Labor', 'Payments', 'Documents'].includes(activeTab) && (
                        <div>
                            <div className="md:hidden space-y-3">
                                        ...
                                        {(Array.isArray(project.laborRecords) ? project.laborRecords.length : 0) === 0 ? <EmptyState message="Lama diiwaan gelin wax shaqaale ah." /> : (project.laborRecords || []).map(lab => (
                                            <div key={lab.id} className='bg-lightGray/50 p-3 rounded-lg'>
                                                <div className='flex justify-between items-start'>
                                                    ...
                                                    <p className='font-bold text-lg text-accent'>${(typeof lab.agreedWage === 'number' ? lab.agreedWage : parseFloat(lab.agreedWage as any) || 0).toLocaleString()}</p>
                                                </div>
                                                {lab.remainingWage > 0 && <p className='text-right text-xs text-redError mt-1'>Hadhay: ${(typeof lab.remainingWage === 'number' ? lab.remainingWage : parseFloat(lab.remainingWage as any) || 0).toLocaleString()}</p>}
                                            </div>
                                        ))}
                                    </>
                                )}
                                    ...
                                    <>
                                        ...
                                        {(Array.isArray(project.payments) ? project.payments.length : 0) === 0 ? <EmptyState message="Lama diiwaan gelin wax lacag bixin ah." /> : (project.payments || []).map(p => (
                                            <div key={p.id} className='bg-lightGray/50 p-3 rounded-lg flex justify-between items-center'>
                                                ...
                                                <p className='font-bold text-lg text-secondary'>+${(typeof p.amount === 'number' ? p.amount : parseFloat(p.amount as any) || 0).toLocaleString()}</p>
                                            </div>
                                        ))}
                                    </>
                                )}
                                {activeTab === 'Documents' && (
                                    <>
                                        <h4 className="font-bold text-lg mb-2 text-gray-700">Kharashaadka Dukumentiyada (Documents)</h4>
                                        {project.expenses.filter(e => e.category && e.category.toLowerCase() === 'document').length === 0 ? (
                                            <EmptyState message="Lama diiwaan gelin wax kharash dukumenti ah." />
                                        ) : (
                                            <div className="space-y-2 mb-6">
                                                {project.expenses.filter(e => e.category && e.category.toLowerCase() === 'document').map(exp => (
                                                    <div key={exp.id} className="bg-white p-3 rounded-lg flex justify-between items-center shadow-sm">
                                                        <div>
                                                            <p className="font-bold">{exp.description}</p>
                                                            <p className="text-xs text-mediumGray">{new Date(exp.expenseDate).toLocaleDateString()}</p>
                                        ...
                                        <h4 className="font-bold text-lg mb-2 text-primary">Dukumentiyada</h4>
                                        {(Array.isArray(project.documents) ? project.documents.length : 0) === 0 ? <EmptyState message="Lama soo gelin wax dukumenti ah." /> : (project.documents || []).map(doc => (
                                            <div key={doc.id} className='bg-lightGray/50 p-3 rounded-lg flex justify-between items-center'>
                                                <div>
                                                    <p className='font-bold'>{doc.name || '-'}</p>
                                                    <p className='text-xs text-mediumGray'>{doc.fileType || '-'}</p>
                                                </div>
                                                <Link href={doc.fileUrl || '#'} target="_blank" className='p-2'><Download size={20}/></Link>
                                            </div>
                                        ))}
                                    </>
                                )}
                            ...
                            <div className="hidden md:block">
                                <table className='w-full text-sm'>
                                    ...
                                    {activeTab === 'Documents' && (project.documents.length > 0 && <thead><tr className='text-left bg-lightGray'><th>Magaca Faylka</th><th>Nooca</th><th>La Soo Galiyey</th><th></th></tr></thead>)}
                                    <tbody>
                                        {activeTab === 'Labor' && (
                                            <>
                                                {/* Expenses for Labor */}
                                                {project.expenses.filter(e => e.category && e.category.toLowerCase() === 'labor').map(exp => (
                                                    <tr key={exp.id + '-exp'}>
                                                        ...
                                                        <td colSpan={2} className="text-right font-semibold text-redError">-${(typeof exp.amount === 'number' ? exp.amount : parseFloat(exp.amount as any) || 0).toLocaleString()}</td>
                                                    </tr>
                                                ))}
                                                {/* Labor Records */}
                                                {(Array.isArray(project.laborRecords) ? project.laborRecords.length : 0) === 0 ? <tr><td colSpan={4}><EmptyState message="Lama diiwaan gelin wax shaqaale ah." /></td></tr> : (project.laborRecords || []).map(lab=>(<tr key={lab.id}><td>{lab.employeeName || '-'}</td><td>{lab.workDescription || '-'}</td><td>${(typeof lab.agreedWage === 'number' ? lab.agreedWage : parseFloat(lab.agreedWage as any) || 0).toLocaleString()}</td><td className='text-right font-semibold text-redError'>${(typeof lab.remainingWage === 'number' ? lab.remainingWage : parseFloat(lab.remainingWage as any) || 0).toLocaleString()}</td></tr>))}
                                            </>
                                        )}
                                        {activeTab === 'Payments' && (
                                            <>
                                                ...
                                                {/* Payments Records */}
                                                {(Array.isArray(project.payments) ? project.payments.length : 0) === 0 ? <tr><td colSpan={4}><EmptyState message="Lama diiwaan gelin wax lacag bixin ah." /></td></tr> : (project.payments || []).map(p=>(<tr key={p.id}><td>{p.paymentDate ? new Date(p.paymentDate).toLocaleDateString() : '-'}</td><td>{p.paymentType || '-'}</td><td>{p.receivedIn || '-'}</td><td className='text-right font-semibold text-secondary'>+${(typeof p.amount === 'number' ? p.amount : parseFloat(p.amount as any) || 0).toLocaleString()}</td></tr>))}
                                            </>
                                        )}
                                        {activeTab === 'Documents' && (
                                            <>
                                                {/* Expenses for Documents */}
                                                {project.expenses.filter(e => e.category && e.category.toLowerCase() === 'document').map(exp => (
                                                    <tr key={exp.id + '-exp'}>
                                                        <td colSpan={3} className="font-bold text-gray-700">{exp.description}</td>
                                                        <td className="text-right font-semibold text-redError">-${(typeof exp.amount === 'number' ? exp.amount : parseFloat(exp.amount as any) || 0).toLocaleString()}</td>
                                                    </tr>
                                                ))}
                                                {/* Documents Records */}
                                                {(Array.isArray(project.documents) ? project.documents.length : 0) === 0 ? (
                                                    <tr><td colSpan={4}><EmptyState message="Lama soo gelin wax dukumenti ah." /></td></tr>
                                                ) : (
                                                    (project.documents || []).map(doc=>(<tr key={doc.id}><td>{doc.name || '-'}</td><td>{doc.fileType || '-'}</td><td>{doc.uploadedAt ? new Date(doc.uploadedAt).toLocaleDateString() : '-'}</td><td className='text-right'><Link href={doc.fileUrl || '#'} target="_blank" className='p-2 text-primary'><Download size={20}/></Link></td></tr>))
                                                )}
                                            </>
                                        )}
                                    </tbody>
