...
export default function BankCashFlowReportPage() {
  ...
  useEffect(() => {
    async function fetchBankReport() {
      setLoading(true);
      setError(null);
      try {
  ...
  const totalBankBalance = accounts.filter(acc => acc.type === 'BANK' || acc.type === 'MOBILE_MONEY').reduce((sum, acc) => sum + acc.balance, 0);
  const totalCashBalance = accounts.filter(acc => acc.type === 'CASH').reduce((sum, acc) => sum + acc.balance, 0);
  ...
  const accountNames = ['All', ...accounts.map(acc => acc.name)];
  const dateRanges = ['All', 'Today', 'Last 7 Days', 'Last 30 Days', 'This Month', 'This Quarter', 'This Year'];

  // Chart data
  const monthlyCashFlowData = aggregateMonthlyCashFlow(filteredTransactions);
  const accountDistributionData = accounts.map(acc => ({ name: acc.name, value: acc.balance })).filter(item => item.value > 0);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden animate-fade-in-up">
        ...
        <div className="p-6 flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 border-b border-lightGray dark:border-gray-700">
          ...
          <div className="relative w-full md:w-48">
            ...
            <select value={filterAccount} onChange={(e) => setFilterAccount(e.target.value)} className="w-full p-3 pl-10 border border-lightGray dark:border-gray-700 rounded-lg bg-lightGray dark:bg-gray-700 text-darkGray dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition duration-200 appearance-none">
