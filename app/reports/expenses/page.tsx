...
export default function ExpensesPage() {
  ...
  const projects = ['All', 'Internal', 'Furniture Project A', 'Office Setup B', 'Restaurant Decor C']; 
  const paidFromOptions = ['All', 'CBE', 'Ebirr', 'Cash'];
  ...
  <td className="p-4 whitespace-nowrap text-darkGray dark:text-gray-100 font-medium flex items-center space-x-2">
    ...
    <td className="p-4 whitespace-nowrap text-right">
      <div className="flex items-center justify-end space-x-2">
        ...
        <button className="p-2 rounded-full bg-accent/10 text-accent hover:bg-accent hover:text-white transition-colors duration-200" title="Edit Expense">
          <Edit size={18} />
        </button>
        <button className="p-2 rounded-full bg-redError/10 text-redError hover:bg-redError hover:text-white transition-colors duration-200" title="Delete Expense">
          <Trash2 size={18} />
        </button>
      </div>
    </td>
...
const ExpenseCard: React.FC<{ expense: typeof dummyExpenses[0]; onEdit: (id: string) => void; onDelete: (id: string) => void }> = ({ expense, onEdit, onDelete }) => (
    <div className={`bg-white dark:bg-gray-800 p-5 rounded-xl shadow-md animate-fade-in-up border-l-4 border-redError relative`}>
        <div className="flex justify-between items-start mb-3">
            <h4 className="font-semibold text-darkGray dark:text-gray-100 text-lg flex items-center space-x-2">
                <DollarSign size={20} className="text-redError"/> <span>{expense.description}</span>
            </h4>
    ...
    <p className="text-sm text-mediumGray dark:text-gray-400 mb-1 flex items-center space-x-2">
      <Tag size={14} className="text-secondary"/> <span>Nooca: {typeof expense.category === 'string' ? expense.category : (expense.category?.name || '')}</span>
        ...
        <p className="text-sm text-mediumGray dark:text-gray-400 mt-2 p-2 bg-lightGray dark:bg-gray-700 rounded-lg border border-lightGray dark:border-gray-600">
  ...
  return (
    <Layout>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-4xl font-bold text-darkGray dark:text-gray-100">Expenses</h1>
        <div className="flex space-x-3">
            <Link href="/expenses/import" className="bg-primary text-white py-2.5 px-6 rounded-lg font-bold text-lg hover:bg-blue-700 transition duration-200 shadow-md flex items-center">
                <Upload className="mr-2" size={20} /> Import Bulk
            </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 animate-fade-in-up">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md text-center">
              <h4 className="text-lg font-semibold text-mediumGray dark:text-gray-400">Kharashyada La Ansixiyay</h4>
              <p className="text-3xl font-extrabold text-secondary">-${approvedExpensesAmount.toLocaleString()}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md text-center">
              <h4 className="text-lg font-semibold text-mediumGray dark:text-gray-400">Sugaya Ansixinta</h4>
              <p className="text-3xl font-extrabold text-accent">{pendingApprovalCount}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md text-center">
              <h4 className="text-lg font-semibold text-mediumGray dark:text-gray-400">Wadarta Diiwaanka</h4>
              <p className="text-3xl font-extrabold text-primary">{expensesCount}</p>
          </div>
      </div>
