...
export async function getSessionCompanyUser() {
  ...
  if (!session.user?.companyId) {
    throw new Error("Session missing companyId. Make sure user is associated with a company.");
  }
  if (!session.user?.id) {
    throw new Error("Session missing userId. Make sure user is authenticated.");
  }
