        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <div className="relative mt-1">
                    ...
                    <input type="text" readOnly value={expense.category} className="w-full p-2 pl-9 rounded-md bg-lightGray dark:bg-gray-700 cursor-not-allowed border-none"/>
                </div>
            </div>
             {/* Date */}
            <div>
                <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Taariikhda</label>
                 <div className="relative mt-1">
                    ...
                    <input type="date" value={expense.date} onChange={e => handleInputChange('date', e.target.value)} className="w-full p-2 pl-9 rounded-md bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600"/>
                </div>
            </div>
             {/* Paid From */}
            <div>
                <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Laga Bixiyay</label>
                 <div className="relative mt-1">
                    ...
                    <select value={expense.paidFrom} onChange={e => handleInputChange('paidFrom', e.target.value)} className="w-full p-2 pl-9 rounded-md bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 appearance-none">
                ...
                <div>
                    ...
                    {expense.materials.map((mat: any, index: number) => (
                        <div key={index} className="grid grid-cols-10 gap-2 mb-2 items-center">
                            ...
                            <button type="button" onClick={() => removeMaterial(index)} className="col-span-1 text-redError hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full p-1 transition-colors"><MinusCircle/></button>
                        </div>
                    ))}
                    <button type="button" onClick={addMaterial} className="text-primary font-semibold flex items-center mt-2 p-1"><Plus size={16} className="mr-1"/>Ku dar Alaab</button>
                </div>
            ) : (
                <div>
                    ...
                    <div className="relative mt-1">
                        ...
                        <input type="number" value={expense.amount} onChange={e => handleInputChange('amount', parseFloat(e.target.value) || 0)} className="w-full p-2 pl-9 rounded-md bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600"/>
                    </div>
                </div>
            )}
        </div>
